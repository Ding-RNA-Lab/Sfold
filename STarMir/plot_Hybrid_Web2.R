...
# is created using the prefix given as an argument.
#
# Modifications are
...
#
# usage plot_Hybrid_Web.R <Output file prefix> <Target Name> <miRNA Name> <site position> <Tar MisMat> <Tar Mat> <miRNA Mat> <miRNA MisMat> <dG Hybrid>
#
# 2013/4/26 (war)
# Altered to accept an output filename rather than a site id.  It makes the script
# more generic.
#
...
plot_Hybrid =function(site,startX=0.1,startY=0.5){
    # Example for input
    #                   Output filename	site[1]: foo.png
    ...
    # increase resolution on png files.
    # ppi = 300;
    
    png(file=site[1]);
    # png(file=site[1], width=4*ppi, height=4*ppi, res=ppi);
    ...
    #for helix	and loop
    titleStartX = startX;
    titleTopY = startY+8*heiPair;
    titleBotY = startY-10*heiPair;
   ...
   #for right end	
    ...
    # this command plots the miRNA name
    text((titleStartX+titleEndX)/2,titleTopY,site[3],cex=fontCex);
    
    points((titleStartX+3*titleEndX)/4,max(titleTopY,posTrack_mir[2,count_mir]+heiPair)+5*heiLoop,pch=2,cex=wcCex);
    text((titleStartX+3*titleEndX)/4+0.5*widPair,max(titleTopY,posTrack_mir[2,count_mir]+heiPair)+5*heiLoop,paste("G=",site[9],"kcal/mol"),cex=fontCex,pos=4,offset=0);

    # this command plots the mRNA name
    text((titleStartX+titleEndX)/2,titleBotY,site[2],cex=fontCex*0.9);		
